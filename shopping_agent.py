...
def search_products(query: str, max_price: Optional[float] = None, is_organic: Optional[bool] = None) -> str:
    ...
    """
    conn = sqlite3.connect(DB_PATH)  ##connectionn of data base 
    cursor = conn.cursor()
    ...
    if query:
        sql += " AND (name LIKE ? OR description LIKE ? OR category LIKE ?)" ##seachig over categoryy
        like = f"%{query}%"
...
# def describe_product_image(image_path: str) -> str:
    """
    Analyze a product image and return product information as JSON.
    """
            ...
            content=[
                {
                    "type": "image_url",
                    "image_url": {
                        "url": f"data:{mime_types[ext]};base64,{image_data}"
                    }
                },
                {
                    "type": "text",
...

{
    ...
    "search_query": "honey",
    "is_organic": true,
    "description": "A jar of organic honey."
}

Rules:
- product_type = type of product
- search_query = short keyword for searching the database
- is_organic = true if organic, false if not organic, null if unclear
...
agent = create_agent(
    ...
    system_prompt=(
        ...
        "7 only add organic and unorgaic lables on food item "
    ),
)
